...
class AMLinear(nn.Module):
    def __init__(self, in_features, n_classes, m=0.35, s=30):
        super().__init__()
        self.weight = nn.Parameter(torch.Tensor(in_features, n_classes))
...
class SimpleSplit(nn.Module):
    ...
    def forward(self, x):
        return torch.cat(
            [x[:, :, start:end, :] for start, end in self.split_indexes],
            axis=1
        )
...
# class SimpleSplit(nn.Module):
#     def __init__(self, patch_size, patch_overlap, feature_dim):
#         super().__init__()
#         self.split_indexes = []
#         start = 0
...
#         while end < 40:
#             print(start, end)
#             start = end - patch_overlap
#             end = start + patch_size
#             self.split_indexes.append([start, end])
#         if self.split_indexes[-1][-1] != feature_dim + 1:
#             raise ValueError('some feats are not used')
#         # self.split_indexes = split_indexes
#
#     def forward(self, x):
#         return torch.cat(
...
def calc_split_index(patch_size, patch_overlap, feature_dim):
    split_indexes = []
    start = 0
    end = start + patch_size
    split_indexes.append([start, end])
    while end - patch_overlap + patch_size <= feature_dim:
        start = end - patch_overlap
        end = start + patch_size
        # print(start, end)
        split_indexes.append([start, end])
    # print(split_indexes)
    if split_indexes[-1][-1] != feature_dim:
        print('last split dim is ', split_indexes[-1][-1])
        print('mfcc dim is ', feature_dim)
        raise ValueError('some feats are not used')
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers,
        ...
        if split_patch_size:
            split_index = calc_split_index(split_patch_size, split_overlap, mfcc_dim)
        else:
            split_index = None
        ...
        if not split_index:
            last_dim = mfcc_dim//5*w_base*10*block.expansion
        elif n_split == 2:
            # last_dim = 3072
            last_dim = 12288
            # last_dim = 6528
            ...
            raise ValueError
        # last_dim=2560
        self.fc = nn.Linear(last_dim, embedding_size)
...
class ResNetDePloy_openvino(ResNet):
...
def _resnet_deploy_openvino(arch, block, layers, pretrained, progress, **kwargs):
    model = ResNetDePloy_openvino(block, layers, **kwargs)
    # if pretrained:
    #     state_dict = load_state_dict_from_url(model_urls[arch],
    #                                           progress=progress)
    #     model.load_state_dict(state_dict)
...
def resnet101deploy_openvino(pretrained=False, progress=True, **kwargs):
    r"""ResNet-101 model from
    `"Deep Residual Learning for Image Recognition" <https://arxiv.org/pdf/1512.03385.pdf>`_
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _resnet_deploy_openvino('resnet101', Bottleneck, [3, 4, 23, 3], pretrained, progress,
                   **kwargs)
...
if __name__ == '__main__':
    m = resnet101(n_classes=1, m=0.35, split_conv=True)
    print(m)
    x = torch.randn(1, 40, 400)
    m(x)
